...
function warningPopup(slotId) {
    // Only create a warning popup if there is *actually* a Pokemon occupying the slot
    if(slots.find(s => s.id === slotId).pokemon != null)
    {
    ...
    // If the slot is empty, you can just remove it (you're not losing anything important)
    else {
        // Remove the slot
        removeSlot(slotId);
        // Deletes the popup (if it exists)
        document.getElementById(`warning-popup-${slotId}`).remove();
    }
}